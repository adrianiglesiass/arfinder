...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("order", sa.Integer(), nullable=True, server_default=sa.text("0")),
        sa.Column("is_main", sa.Boolean(), nullable=True, server_default=sa.text("0")),
        sa.Column(
            "created_at", sa.DateTime(), server_default=sa.text("CURRENT_TIMESTAMP")
        ),
    )
    op.create_index("idx_profile_order", "profile_photo", ["profile_id", "order"])
...
def downgrade() -> None:
    ...
    try:
        op.add_column(
            "profile", sa.Column("foto_url", sa.String(length=500), nullable=True)
        )
    except Exception:
        pass
